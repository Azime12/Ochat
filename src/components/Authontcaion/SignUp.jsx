...
const SighUp = () => {
  ...
  const [responsiblity, setIsSuperAdmin] = useState();
  const [phoneNumber, setPhoneNember] = useState();
  ...
  const submitHandler = async () => {
    setPicLoading(true);
    if (!name || !phoneNumber || !password || !confirmpassword||!responsiblity) {
      toast({
        title: "ሁሉንም ቅጽ አልሞሉም!!",
        status: "warning",
        duration: 5000,
    ...
    var re = new RegExp("^[+]?[(]?[0-9]{3}[)]?[.]?[0-9]{3}[-.]?[0-9]{4,6}$");
    ...
    console.log(name, phoneNumber, password,responsiblity, pic);
    try {
  ...
  return (
	...
	<Radio colorScheme='green' value='user' onChange={(e)=>setIsSuperAdmin(e.target.value)}>
      Leader
    </Radio>
  </Stack>
 </RadioGroup>
